...
def convert_pred(
        ...
        def check_status() :
            ...
            while count <= total_num : 
                ...
                string += "[{}/{}]".format(
                    str(count).zfill(3), str(total_num).zfill(3)
                )
        ...
        pred_generator = PredGenerator(
            input_shape, src_path, pred_dir, pattern,
            n_fft, win_length, sample_rate
        )
        
        try : model.predict(
                np.array([np.zeros(pred_generator.input_shape)]), 
                verbose=False
        )
        ...
        while not pred_generator._is_all_done : 
            ...
            for bulk_arr in pred_data : 
                ...
                while bulk_pivot < bulk_max_pivot : 
                    alter_pivot = 64
                    total_arr[:,pivot:pivot + alter_pivot] = polar_to_complex(bulk_arr[
                        :,bulk_pivot:bulk_pivot + alter_pivot
                    ])
