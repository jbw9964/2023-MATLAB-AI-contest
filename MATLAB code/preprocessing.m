...
function D_merge_data = preprocessing(test_data,input_shape,n_fft,win_length)
    ...
    for i = 1:(Quotient+1)                                                              % Allocate data to each cell through a loop statement
        ...
        test_data_cell{i} = stft(test_data(start_idx:end_idx), "Window", ...
            hamming(win_length), 'OverlapLength', win_length*3/4, "FFTLength", n_fft);  % STFT conversion for each allocated cell
    end
