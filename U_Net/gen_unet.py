...
def gen_separate_unet(
        input_shape=(960, 832, 2), 
        encode=5, num_separ=2
) -> Model : 
    ...
    for _ in range(num_separ) : 
        ...
        for _ in range(encode) : 
            conv_encoder = Conv2D(
                filters=filter_num, kernel_size=5, strides=2, 
                padding="same"
            )(last_layer)
        ...
        for concate_layer in concate_list : 
            ...
            conv_decoder = Conv2D(
                filters=filter_num, kernel_size=5, strides=1, 
                padding="same"
            )(merge_layer)
        ...
        conv_layer = Conv2DTranspose(
            filters=8 * input_shape[2], kernel_size=5, strides=2, 
            padding="same"
        )(last_layer)
        conv_layer = Conv2DTranspose(
            filters=input_shape[2], kernel_size=3, strides=1, 
    ...
    for iteration, inst_layer in enumerate(subtract_list) : 
        vocal_separation_layer = Subtract(name="sub_{}".format(iteration + 1))([
            vocal_separation_layer, inst_layer
        ])
